...
export default function DeepDivePage({ lang, setLang }: DeepDivePageProps) {
        ...
        const loadParentData = async () => {
            try {
                ...
                if (!data || !data.analysis_result) {
                    setError(lang === 'es'
                        ? 'El análisis de empresa debe estar completado antes de hacer un Deep Dive.'
                        : 'The business analysis must be completed before starting a Deep Dive.');
                    return;
                }
                ...
                // Extract onboarding data but strip heavy fields (images, documents)
    ...
    const handleFormComplete = async (data: DeepDiveInput) => {
        setIsSaving(true);
        try {
            // Strip base64 images before saving to DB
            const { productImages, ...dataWithoutImages } = data;

            const { data: insertedData, error } = await supabase
                .from('product_analyses')
                .insert({
                    business_report_id: reportId,
                    product_name: data.productName || 'Producto sin nombre',
                    status: 'draft',
                    product_input_data: dataWithoutImages,
                    is_paid: false,
                    user_id: user!.id
                })
                .select()
            ...
            navigate(`/deep-dive/checkout/${insertedData.id}`);
        } catch (error) {
            console.error('Error saving deep dive draft:', error);
            ...
            {isSaving && (
                <div className="fixed inset-0 bg-white/80 backdrop-blur-sm z-50 flex flex-col items-center justify-center">
                    ...
                    <p className="text-xl font-bold text-slate-800">
                        {lang === 'es' ? 'Guardando tu estrategia...' : 'Saving your strategy...'}
                    </p>
                </div>
            )}
        </div>
